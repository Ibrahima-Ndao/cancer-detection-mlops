...
def run_eval(cfg, logger, model_name, weights_path, img_size=None):
    ...
    if platform.system().lower().startswith("win") and workers > 0:
        workers = 0  # Windows: si souci multiprocess

    # charge uniquement le loader de validation
    ...
    logger.info(f"[EVAL {model_name}] "
                f"AUC={m['auc']:.4f} ACC={m['accuracy']:.4f} "
                f"P={m['precision']:.4f} R={m['recall']:.4f} F1={m['f1']:.4f}")

    # ---- MLflow (run d'éval séparé)
    ...
    # ---- Ecriture JSON pour DVC (si demandé)
    out_json = os.environ.get("EVAL_JSON")
    if out_json:
        out_dir = os.path.dirname(out_json)
        if out_dir:  # si "reports/metrics.json", crée "reports"
